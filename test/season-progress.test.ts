...
describe("season progress", () => {
  ...
  it("returns zero before any result exists", () => {
    ...
    ).toMatchObject({ completedRaces: 0, remainingRaces: 1, completionPercentage: 0 });
  });
  ...
  it("returns one hundred percent after the final result", () => {
    const result = calculateSeasonProgress(
      schedule([
        ...
        race({ round: "2", startDate: "2026-12-06T14:00:00.000Z" }),
      ]),
      latestResults("2"),
    );
    expect(result).toMatchObject({ completedRaces: 2, remainingRaces: 0, completionPercentage: 100 });
  });
});
